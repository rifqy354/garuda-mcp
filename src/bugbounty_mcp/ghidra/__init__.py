...
'''
    ...
    async def analyze(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Build the script
            class_path = str(project_dir)
            compile_cmd = [
                "javac",
                "-cp", f"{self.settings.install_path}/GhidraAPI/:{self.settings.install_path}/support/lib/*.jar",
                "-d", class_path,
                str(script_file)
            ]
            ...
            # Add output file property
            cmd.insert(0, f"-Doutput.file={output_file}")
    ...
    async def decompile_function(
        ...
        # Similar to analyze but with specific function decompilation
        output = await self.analyze(binary_path)

        for func in output.get("functions", []):
            if func.get("name") == function_name:
                # Return decompiled code if available
    ...
    async def get_binary_info(self, binary_path: str) -> BinaryInfo:
        ...
        for line in readelf_output.split("\n"):
            if "Machine:" in line:
                ...
                entry_point = line.split("Entry point")[1].strip()
            elif "Flags:" in line:
                if "0x" in line:
